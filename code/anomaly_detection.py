...
def plot_all(y, anom_if, scores_if, anom_svm, scores_svm, save_dir="results"):
...
def save_results(y, anom_if, anom_svm, agree, ap_if, ap_svm, save_dir="results"):
    """Save anomaly detection summary to JSON."""
    os.makedirs(save_dir, exist_ok=True)
    baseline = y.mean()
    ...
    summary = {
        ...
        "overall_fake_ratio": round(float(baseline), 4),
    }

    with open(f"{save_dir}/anomaly_detection_results.json", "w") as f:
        json.dump(summary, f, indent=4)
    print("[SAVED] anomaly_detection_results.json")
